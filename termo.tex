...
\begin{frame}[c]{Terceira lei da Termodinâmica}
 ...
 % \begin{frame}{Mecânica estatística}
 %     \begin{block}{}
 %         A termodinâmica trata das conclusões que podem ser tiradas de certas leis
 %         experimentais e das aplicações destas conclusões a relações entre
 %         propriedades de materiais, como capacidades térmicas, coeficientes de
 %         expansão, compressibilidades e assim por diante. Ela não faz hipóteses
 %         sobre a natureza da matéria e é uma ciência puramente empírica.
 %     \end{block}
 %     \pause

 %     \begin{block}{}
 %         Embora os princípios da termodinâmica possam predizer muitas relações
 %         entre as propriedades da matéria, não é possível deduzir de considerações exclusivamente
 %         termodinâmicas a equação de estado de uma substância
 %     \end{block}
 %     \pause

 %     \begin{block}{}
 %         Só podemos transpor as limitações da termodinâmica pura, fazendo hipóteses
 %         quanto à natureza da matéria; e, sem dúvida, a mais fértil destas
 %         hipóteses, bem como uma das mais antigas, é que a matéria não é de
 %         estrutura contínua, mas composta de partículas chamadas moléculas. Em
 %         particular, a teoria molecular dos gases foi muito desenvolvida, porque os
 %         problemas a resolver são muito mais simples que os relativos a líquidos e
 %         sólidos.
 %     \end{block}
 % \end{frame}
 \begin{frame}{Atividade valendo a \(N_3\)}
     \begin{itemize}
         \item Os alunos da disciplina devem se dividir em 4 grupos e cada grupo deve escolher um tema para
             ...
             \end{enumerate}
         \item A participação nas apresentações (ouvinte ou apresentador) vale 50\% da \(N_3\) (nota individual)
         \item A \textit{qualidade} da apresentação vale 50\% da \(N_3\) (nota do grupo)
